...
wget https://github.com/ipmanlk/breeze/releases/latest/download/breeze-linux-amd64
chmod +x breeze-linux-amd64
mv breeze-linux-amd64 /usr/local/bin/breeze
```

## Option 2: Build from Source

```bash
git clone https://github.com/ipmanlk/breeze.git
cd breeze
make build          # Builds ui/dist then compiles Go binary
# Binary at: bin/breeze
```
...
runs Breeze with a named volume for the database and uploads:
...
- **Named volume `breeze-data`** mounted at `/data`, so the SQLite database and
...
- Expose a different host port by setting `BREEZE_HTTP_PORT` in your
...
Create `/etc/systemd/system/breeze.service`:

```ini
[Unit]
Description=Breeze Project Manager
...
Environment="DB_PATH=/var/lib/breeze/breeze.db"
Environment="UPLOAD_DIR=/var/lib/breeze/uploads"
ExecStart=/usr/local/bin/breeze
Restart=on-failure
RestartSec=5
...
```bash
sudo useradd -r -s /bin/false breeze
sudo mkdir -p /var/lib/breeze
sudo chown breeze:breeze /var/lib/breeze
sudo systemctl daemon-reload
sudo systemctl enable --now breeze
```
...
curl -fsS -b cookies.txt \
  https://breeze.example.com/api/backup/download \
  -o /backups/breeze-$(date +%Y%m%d).db

# Backup uploads
rsync -av /var/lib/breeze/uploads/ /backups/uploads/
```
...
filesystem directly, stop Breeze first or use `sqlite3 /path/db ".backup"`.

## Upgrading

1. Stop the service: `sudo systemctl stop breeze`
2. Backup the database (see above)
3. Replace the binary: `cp breeze /usr/local/bin/breeze`
4. Start the service: `sudo systemctl start breeze`
